	 ...
	 * 요청시 전달한 값(파라미터)를 받는 방법
	   1. HttpServletRequest를 이용한 방법
	   	  ...
	   	  public String detailView(HttpServletRequest request) {
	   	   int bno = Integer.parseInt(request.getParameter("boardno")); // 자동형변환 됨
	   	  }
	   
	   2. @RequestParam 어노테이션을 사용하는 방법
	   		: 매서드 위에 어노테이션을 넣는 방법
	   			- 변수에 저장할 때 : request.getParameter("키")
	   		ex) detail?boardno=1&writer=아무개	
	   		@GetMapping("/detail")
	   ...
	   3. 매개변수명으로 사용하는 방법
	   	  ...
	   	  ex)
	   	  @GetMapping("/detail")
	   		 public String detailView(int boardno, String writer) {
	   ...
	   4. @ModelAttribute 어노테이션을 사용하는 방법
	   	  ...
	   	  *** 반드시 name과 담고자하는 필드명이 동일 해야 됨 ***  <input name="boardno"> ==> Board bean 파일과 필드명 동일해야 함
	   		ex)
	   		@GetMapping("/detail")
