...
public class MenuRestController {
	...
	// /menu/type/KR, /menu/type/CN, /menu/type/JP
	...
	public Menu findById(@PathVariable(name="id") Long id) {
		...
		// 없는 id를 넣어서 오류 났을 때 500(서버측 오류)가 뜸
		// 사용자의 잘못인 것(400~번대의 오류가 나야함) 근데 500번대로 뜸
	}
	*/
	/*
	 * ResponseEntity : 결과 데이터와 HTTP상태코드와 오류코드까지 직접 제어할 수 있는 클래스
	   ...
	   - body : 메세지 body에 작성될 내용(요구사항의 대응)
	...
	@PostMapping()
	public ResponseEntity<?> insertMenu(@RequestBody Menu menu) { //@RequestBody JSON으로 넘어온것을 자바객체로 변경해서 전달(Bean.class처럼)
	...
	public ResponseEntity<?> deleteMenu(@PathVariable(name="id") Long id) {
		...
		return ResponseEntity.noContent().build(); // 딜리트하면 값이 없으니깐 http 상태코드 204 노 컨텐츠
		}
	}
